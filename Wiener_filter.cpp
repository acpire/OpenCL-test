...
Wiener_filter::Wiener_filter(clDevice* device, cl_uchar4* image, size_t width, size_t height, cl_float4* kernel, size_t width_kernel, size_t height_kernel)
{
	...
	cl_uint spectrum_kernel_gpu = device->mallocImageMemory(&null_ptr, &height_kernel, &width_kernel, length_row_pitch_data, 1, _type_image, _type_data);
	...
	cl_uint mean_image_gpu = device->mallocImageMemory(&null_ptr, &one_to_one_image, &one_to_one_image, length_row_pitch_data + 4, 1, _type_image, _type_data);
